...
test('production manifest and icons satisfy PWA metadata requirements', async () => {
  ...
  const favicon = await readFile(path.join(root, 'dist', 'favicon.ico'))
  assert.equal(favicon.readUInt16LE(0), 0)
  assert.equal(favicon.readUInt16LE(2), 1)
  assert.equal(favicon.readUInt16LE(4), 1)
  assert.equal(favicon[6], 48)
  assert.equal(favicon[7], 48)
  assert.deepEqual(pngSize(favicon.subarray(favicon.readUInt32LE(18))), { width: 48, height: 48 })
  assert.deepEqual(pngSize(await readFile(path.join(root, 'dist', 'favicon-vmt-gold-48.png'))), { width: 48, height: 48 })
  ...
  assert.match(html, /rel="icon" type="image\/png" sizes="48x48" href="\/favicon-vmt-gold-48\.png"/)
  assert.doesNotMatch(html, /rel="icon"[^>]+href="[^"]+"[^>]*>\s*<link rel="icon"/)
